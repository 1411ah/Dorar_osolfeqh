...
def scrape_special_page(url: str, pid: str, title: str, level: int = 1) -> "Page | None":
    ...
    return Page(
        ...
        breadcrumb=[BOOK_TITLE, title], body_html=body_html, footnotes=footnotes,
    )
...
def _extract_refs_content(soup: BeautifulSoup) -> str:
    ...
    if not cntnt:
        return ""
    body = cntnt.find("div", class_=lambda c: c and "w-100" in c and "mt-4" in c) or cntnt
    ...
    for article in body.find_all("article"):
        ...
        for div in article.find_all("div", class_="d-block"):
            pairs = [s.get_text(separator=" ", strip=True) for s in div.find_all("strong")]
            if pairs:
                details.append(" | ".join(pairs))
        block = f"<p><strong>{title}</strong> {' | '.join(details)}</p>"
...
def extract_content(soup: BeautifulSoup, pid: str) -> tuple[str, list[tuple[str, str]]]:
    ...
    # ابحث عن حاوية المحتوى الفعلية داخل cntnt
    body = cntnt.find("div", class_=lambda c: c and "w-100" in c and "mt-4" in c) or cntnt
    body = BeautifulSoup(str(body), "html.parser")
...
def export_markdown(items: list[Item]) -> None:
    ...
    for item in items:
        if not isinstance(item, IndexPage):
            continue
        matched = next(
            (dpath for key, dpath in section_dirs.items()
             if len(key) == item.level and key[-1] == item.title),
            MD_DIR / safe_name(item.title),
        )
...
def _nav_xhtml(entries: list[tuple]) -> str:
    inner = "\n".join(_render_nav_ol(_build_toc_tree(entries)))
    return f"""\
<?xml version="1.0" encoding="utf-8"?>
...
def export_epub(items: list[Item]) -> None:
    ...
    with zipfile.ZipFile(EPUB_PATH, "w", zipfile.ZIP_DEFLATED) as zf:
        ...
        for item in items:
            ...
            zf.writestr(
                f"OEBPS/pages/{fn}",
                _page_xhtml(item) if isinstance(item, Page) else _index_xhtml(item),
            )
            man_items.append(
        ...
        po      = [0]
        ncx_pts = "\n".join(_render_ncx(_build_toc_tree(toc_entries), po))
...
def main() -> None:
    ...
    for i, (title, url) in enumerate(FRONT_PAGES, 1):
        p = scrape_special_page(url, f"front{i:02d}", title, level=1)
        if p:
            front.append(p)
    ...
    for i, (title, url) in enumerate(BACK_PAGES, 1):
        p = scrape_special_page(url, f"back{i:02d}", title, level=1)
        if p:
            back.append(p)

    print("4) بناء الهيكل…")
    items = build_document(front + raw_pages + back)
